...
if (empty($email)) {
    ...
    if (empty($password) || empty($confirm)) {
        ...
        $query = mysqli_query($conn, "UPDATE tb_users SET password='$hash' WHERE email='$email'");
        if (mysqli_affected_rows($conn) > 0) {
            ...
            $message = "<div class='text-danger mb-2'>Gagal reset password.</div>";
        }
    }
}
?>
...
<body>
    ...
    <div class="form-box">
        <h3 class="mb-4">Buat Password Baru</h3>
        <?= $message ?>
        <form method="post">
            <div class="mb-3">
