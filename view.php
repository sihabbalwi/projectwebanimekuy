        ...
        <div class="row gy-3">
            ...
            <div class="col-lg-8 mb-4">
                ...
                <div id="comment_section">
                    ...
                    <?php else: ?>
                        <div class="alert alert-warning mt-3">Silakan <a href="/Login/Login.php" class="text-decoration-underline">login dengan akun Google</a> untuk memberikan komentar.</div>
                    <?php endif; ?>
        ...
        function renderKomentar(list) {
            ...
            list.forEach((item) => {
                ...
                <div class="d-flex mb-3" data-id="${item.id_komentar}">
                    <img src="${item.user_avatar}" alt="avatar" class="rounded-circle me-2" width="40" height="40">
                    <div>
                        <strong>@${item.user_nama}</strong> <small class="text-muted">${timeAgo(item.waktu)}</small>
                        <p>${item.isi_komentar}</p>
                        <div>
                            <a href="#" class="me-3 text-muted">👍</a>
                            <a href="#" class="me-3 text-muted">👎</a>
                            <a href="#" class="balas-link text-muted" data-id="${item.id_komentar}">Balas</a>
                        </div>

                        <!-- Tempat untuk form balas -->
                        <div class="form-balas mt-2" id="form-balas-${item.id_komentar}" style="display:none;">
                            <textarea class="form-control mb-2" rows="2" placeholder="Tulis balasan..."></textarea>
                            <button class="btn btn-primary btn-sm kirim-balas" data-parent="${item.id_komentar}">Balas</button>
                            <button class="btn btn-secondary btn-sm batal-balas" data-id="${item.id_komentar}">Batal</button>
                        </div>

                        <!-- Tempat balasan rekursif -->
                        <div class="ms-4 mt-3">
                            ${item.replies && item.replies.length > 0 ? renderKomentar(item.replies) : ""}
                        </div>
                    </div>
                </div>
                `;
